...
<div>
   <hr/>
<p>{props.comment.owner}: {props.comment.description}</p>
    {props.auth.user.id === props.comment.user_id ? 
    <div>
     <Button onClick = {() => props.removeComment(props.comment.id)}>Delete Comment</Button>
     <Button onClick = {() => setEditing(!editing)}>Edit Comment</Button>
     </div> : null }
